"""SAM3 矩形内分割（REGION_SAM）口径实测（W53）。

点击口径（中心点分割）W52 已量化：0.546/0.572/63%（选择 v2 上产）。
矩形内分割（predict_point_in_box）从未量化——本实验 val 162 同口径：
  A. 盒边距扫描 m∈{0,16,64}：GT bbox 外扩，模拟标注员拖框精度
     （m=0 贴紧上界，m 大=手松）；点击锚=GT 质心（与点击口径同锚可比）
  B. 实例选择策略：argmax（现产）vs nearest（W52 点击口径赢家）
两策略共享同一次前向（W52 实验同法）；掩码∩矩形硬约束复刻 W43 语义。

对照表锚点：点击口径 0.546/0.572/63%/零产出 1（exp_sam3_inference_params.py）。
"""
...
def clip_to_box(mask: np.ndarray, box) -> np.ndarray:
    """W43 掩码∩矩形硬约束（predict_point_in_box 同款）。"""
    ...
    for p, h, w, t, cx, cy, bx1, by1, bx2, by2 in targets:
        ...
        if not masks:
            for v in res.values():
                v.append(0.0)
            continue
        # argmax（现产语义：predict_point_in_box 原样复刻）
        ...
        def _d(m, cx=cx, cy=cy):
            ...
            if len(xs2) == 0:
                return 1e18
            return ((xs2.mean() - cx) ** 2 + (ys2.mean() - cy) ** 2) ** 0.5

        best_n = min(masks, key=_d)
        res["nearest"].append(
            poly_iou(_mask_to_polygon(clip_to_box(best_n, box)), t, h, w)
        )
    for k, v in res.items():
        a = np.array(v)
        print(
            f"[m={margin:2} 选择={k:8}] mean={a.mean():.3f} median={np.median(a):.3f} "
            f">=0.5={(a >= 0.5).mean():.0%} 零产出={(a == 0).sum()} "
            f"({time.time()-t0:.0f}s/双策略)",
            flush=True,
        )
