...
**FR-008: 数据工具集次批**（裁剪数据集/照片尾缀修改/数据清洗）| P2
**FR-009: 训练子进程隔离评估与实施**（对标 multiprocessing 子进程训练）| P2 —— 崩溃隔离收益 vs TrainWorker 改造成本，波次前置评估后决定实施与否（待裁决 D-3）。
**FR-010: 超分训练**（对标 super 训练模板）| P2。
