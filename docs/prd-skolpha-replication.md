...
| 切割线（cut_line） | SKolpha 标注 7 形态之一，推断为折线/切割路径形态〔🔎符号级证据 @0x3d52001-0x3d5205b，交互语义待实机核对〕〔**2026-09-02 wave3 修正**：原品真身=「裁剪标注」X 键切分既有图形工具，非折线形态；两点线形标注的真身是「斜线标注」bias_ai_label，见 docs/skolpha-forensics-wave3.md §3.1〕 |
| 操作标注（operation） | SKolpha 标注 7 形态之一，推断为操作区域+操作名形态〔🔎同上〕〔**2026-09-02 wave3 修正**：原品真身=「交互式标注」I 键 SAM 交互分割（≈AVA 既有 INTERACTIVE），非「区域+操作名」，见 wave3 §3.2〕 |
...
**FR-001: 切割线标注形态**（对标 cut_line_label）| P0 ✅ W56 落地 ◐ 原品语义修正（2026-09-02 wave3：cut_line_label 实为「裁剪标注」X 键**切分既有图形工具**，非折线标注形态——推断推翻；AVA CUT_LINE 保留为自有线形标注能力，修订裁决待 D-23，见 wave3 §3.1/§4）
...
**FR-002: 操作标注形态**（对标 operation_label）| P0 ✅ W56 落地 ◐ 原品语义修正（2026-09-02 wave3：operation_label 实为「交互式标注」I 键 SAM 交互分割（≈AVA 既有 INTERACTIVE），非「区域+操作名」——推断推翻；修订裁决待 D-23，见 wave3 §3.2/§4）
...
**FR-003: 批量预测线程模式**（对标 batchPredictThread/batchPredictOnlyOne）| P0 ✅ W56 落地 ✅ 方向证实（2026-09-02 wave3：原品批量预测=整批推理+**labelme JSON 对照逐图/汇总漏检过杀统计**；incremental 为 AVA 原生增量设计原品无对应；对照统计与预标注 JSON 导出记观察项，见 wave3 §3.3）
...
**FR-011: 裁剪标注切分工具**（对标 SKolpha「裁剪标注」X 键/wave3 §3.1）| P2 🆕 2026-09-02 D-23 裁决 B **立项**（规格=chm page_159：画裁剪线切分既有图形——矩形过两条对边→切为两个新矩形〔过两宽取中点 x 替换交点 x、过两高取中点 y〕，多边形边 ≥2 交点→切为若干新图形，原形删除新形生成；实施波次待排）。
**FR-012: 斜线标注**（对标 bias_ai_label「斜线标注」Ctrl+W，wave3 §3.1）| P2 🆕 2026-09-02 D-23 裁决 B **随 FR-011 评估**：两点画穿缺陷斜线→自动标注该缺陷；AVA CUT_LINE 已覆盖线形标注基础形态，立项前评估「两点→自动标注」交互增益是否单列模式。
...
- **AC-010**: 动态核对 ≥2 处（SKolpha 实机：切割线/操作标注交互语义、批量预测模式语义）——用户配合项，静态先行不阻塞〔全局〕→ **✅ 意图达成（2026-09-02 wave3，文档级五源互证：随包快捷键说明/save_json.py/exe 字符串簇对齐/chm 官方手册/版本页；三处全核对——两处推翻一处证实）**；实机 GUI 走查受 UAC+加密狗双门槛留用户可选加强项（wave3 §5），观察脚本已备
...
- 假设用户可配合 ≥1 次 SKolpha 实机动态核对（AC-010）；否则三处符号级语义维持推断级并留档。（**2026-09-02 兑现并超预期**：产品文档级核对完成——两处推翻一处证实，wave3；GUI 级走查因 UAC+加密狗门槛留可选）
...
- 🔮 SKolpha 工程文件 (.spro) 只读导入器（利用已破密钥，若用户需要迁移历史工程）；缺陷样品分类工具；超分训练；批量预测**漏检/过杀对照统计**与**预标注 JSON 导出**（wave3 §3.3 观察项，原品有 AVA 无）
...
|----------|----------|----------|--------|------|
| FR-001 | US-001 切割线标注 | AC-001/002/008/009/010 | P0 | ✅ W56（◐ 原品语义修正见 §3.1/wave3） |
| FR-002 | US-002 操作标注 | AC-001/002/008/009/010 | P0 | ✅ W56（◐ 原品语义修正见 §3.1/wave3） |
| FR-003 | US-003 批量预测模式 | AC-003/008 | P0 | ✅ W56（✅ 方向证实 wave3） |
| FR-004 | US-004 训练模板 | AC-004/008/009 | P1 | ✅ W57 |
| FR-005 | US-005 工程绑定 | AC-005/008 | P1 | ✅ W58 |
| FR-006 | US-006 数据工具 | AC-006/008 | P1 | ✅ W58（首批）+W60（次批） |
| FR-007 | US-007 API 推理 | AC-007/008 | P1 | ✅ W59 |
| FR-008/009/010 | — | AC-008 | P2 | FR-008 ✅ W60；FR-009/010 ⏸️ 缓办（§3.1 裁决） |
| FR-011/012 | —（D-23 裁决 B 立项） | AC-008 | P2 | 后续波次（规格=wave3 §3.1/§4） |
| NFR-001..005 | — | §5.2/AC-008 | P0/P1 | ✅（终态门禁 1324 绿 / lite 1.980GiB / 并发基线 0.96x 不劣化） |
...
- [x] **假设已声明**: §6.0/§6.3 显式
- [x] **编号连续**: FR-001..012、AC-001..010、NFR-001..005 无跳号（FR-011/012 为 D-23 裁决后新增，映射 AC-008）
- [x] **语言一致**: 中文
