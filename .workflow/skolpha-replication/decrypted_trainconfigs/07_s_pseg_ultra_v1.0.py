...
pretrain_load_from = r"./TrainConfigs\pretrain_ultra-pseg.pth"
my_num_classes = 2
...
img_norm_cfg = dict(mean=[103.530, 116.280, 123.675], std=[1.0, 1.0, 1.0])  # 原始
...
val_init_pipeline = [
    dict(type='LoadCategoryList', ignore_labels=['ignore']),
    dict(type='LoadPathList'),
    dict(type='SplitData', start=0.9, end=1, key='json_path_list'),
    dict(type='LoadJsonDataList'),
    dict(type='LoadLabelmeDataset'),
    dict(type='Labelme2Coco'),
    dict(type='StatCategoryCounter'),
...
model = dict(
    ...
    backbone=dict(
        ...
        norm_eval=True,
        style='caffe',
        init_cfg=dict(type='Pretrained', checkpoint=r'./TrainConfigs/resnet50_ultra.pth')),
    neck=dict(
        type='FPN',
