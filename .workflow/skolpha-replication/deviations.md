...
# AC-010 语义核对批偏差记录（2026-09-02，程序级最后一项收口）

| # | 偏差 | 类型 | 处置 |
|---|------|------|------|
| D-4（闭账） | AC-010 原定「用户配合实机动态核对」→ 实际以**产品文档级五源互证**完成（随包快捷键说明 save_json.py/exe 字符串簇对齐/chm 官方手册/版本页），三处全核对：cut_line_label/operation_label **两处推断推翻**、批量预测方向证实 | 执行方式替代（核对意图达成） | 报告 docs/skolpha-forensics-wave3.md；PRD AC-010/FR-001..003/术语表已回填 |
| D-23（待裁决） | 推翻后的修订决策：方案 A=保留 AVA CUT_LINE/OPERATION 为自有能力+文档记录差异（推荐）；B=裁剪标注（X 切分工具，page_159 即规格）立项后续 FR+斜线标注评估；C=按原品语义更名重构（不推荐）。观察项：漏检/过杀对照统计、预标注 JSON 导出 | 新增待裁决项（范围决策属用户） | 不自动实施；wave3 §4 供裁决 |
| D-24 | 实机 GUI 走查未执行——skolpha.exe 需 UAC 提权（WinError 740，证据 live_observe/run.err）且 v3.2.4 起有加密狗门槛；观察脚本 live_observe_ac010.py 已备（只读 UIA 树+截图，需提权改造后由用户在场合跑） | 环境门槛 | 留用户可选加强项；文档级证据已达 AC-010 意图 |

## AC-010 批门禁记录
- 本批零生产码改动（纯取证+文档回填）；主门禁不适用（无代码变更）
- 交付物：docs/skolpha-forensics-wave3.md + PRD 回填 ×6 处 + 本节 + EXP-20260902g

> **D-23 裁决（2026-09-02，用户批准）：A+B** —— A：CUT_LINE/OPERATION 保留为 AVA 自有能力（零代码改动，差异已档 wave3）；B：FR-011 裁剪标注切分工具**立项**、FR-012 斜线标注随其评估（PRD §3.1/§9 已落条目；观察项入 §7.3）。C 否决。


---

# W62 裁剪标注批偏差记录（FR-011，2026-09-02，L2）

| # | 偏差 | 类型 | 处置 |
|---|------|------|------|
| D-25 | **W38 版本守卫存量红发现**：W61「settings milestone 清理」晚于末次全量门禁执行，`v<ver> (M<n>)` 模式失配带红入库（3cbc688 起仓库自带 1 红）——本批全量门禁首跑逮出；守卫模式对齐清理后形态 `"<v<ver><br>"`（一致性不变量不变，仅锚文本随 W61 故意清理更新）。教训 EXP-20260902h：改被守卫锚定的文本后必须重跑含该守卫的门禁再收 | 存量缺陷+守卫口径更新 | 门禁全绿收口；W56 全集守卫补 CROP 属 FB-016 标准增量 |
| D-26 | 实现微调：on_mouse_move/handle_move 两入口函数体同构，CROP 分支以 replace_all 单次编辑双接线；切线语义取「过两点无限直线」（SKolpha 为画线段+严格交点判定）——画不满形状也按延长线切，交互更省心（PRD §5 已留偏差档） | 实现微调 | 25 用例含 14 几何边界全绿 |

## W62 门禁记录
- 新测试 25 用例一次全绿；全量 **1349 passed / 5 skipped / rc=0**；ruff 0 error
